<template>
  <div>
    <h2 id="page-heading" data-cy="JobHistoryHeading">
      <span id="job-history-my-suffix-heading">Job Histories</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon> <span>Refresh List</span>
        </button>
        <router-link :to="{ name: 'JobHistoryMySuffixCreate' }" custom v-slot="{ navigate }">
          <button
            @click="navigate"
            id="jh-create-entity"
            data-cy="entityCreateButton"
            class="btn btn-primary jh-create-entity create-job-history-my-suffix"
          >
      ...
      <table class="table table-striped" aria-describedby="jobHistories">
        ...
        <tbody>
          <tr v-for="jobHistory in jobHistories" :key="jobHistory.id" data-cy="entityTable">
            <td>
              <router-link :to="{ name: 'JobHistoryMySuffixView', params: { jobHistoryId: jobHistory.id } }">{{
                jobHistory.id
              }}</router-link>
            </td>
            ...
            <td>
              <div v-if="jobHistory.job">
                <router-link :to="{ name: 'JobMySuffixView', params: { jobId: jobHistory.job.id } }">{{ jobHistory.job.id }}</router-link>
              </div>
            </td>
            <td>
              <div v-if="jobHistory.department">
                <router-link :to="{ name: 'DepartmentMySuffixView', params: { departmentId: jobHistory.department.id } }">{{
            ...
            <td>
              <div v-if="jobHistory.employee">
                <router-link :to="{ name: 'EmployeeMySuffixView', params: { employeeId: jobHistory.employee.id } }">{{
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link :to="{ name: 'JobHistoryMySuffixView', params: { jobHistoryId: jobHistory.id } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-info btn-sm details" data-cy="entityDetailsButton">
                    <font-awesome-icon icon="eye"></font-awesome-icon>
                    <span class="d-none d-md-inline">View</span>
                  </button>
                </router-link>
                <router-link :to="{ name: 'JobHistoryMySuffixEdit', params: { jobHistoryId: jobHistory.id } }" custom v-slot="{ navigate }">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="test7App.jobHistory.delete.question" data-cy="jobHistoryDeleteDialogHeading">Confirm delete operation</span></span
      >
      <div class="modal-body">
        <p id="jhi-delete-jobHistory-heading">Are you sure you want to delete this Job History?</p>
      </div>
      <div slot="modal-footer">
        ...
        <button
          ...
          v-on:click="removeJobHistoryMySuffix()"
        >
          Delete
        </button>
      </div>
...
<script lang="ts" src="./job-history-my-suffix.component.ts"></script>
